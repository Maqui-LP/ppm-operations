...
{
  if (p.depth >= 256)
    error(1, 0, "Error in header: depth can be %d. Maximum allowed is 256", p.depth);
}
...
t_ppm ppm_from_file(FILE *f)
{
  ...
  unsigned int row, col;
  char ppm_type[] = {0, 0, 0};
  if (fread(ppm_type, sizeof(char), strlen(PPM_TYPE), f) != strlen(PPM_TYPE))
    error(1, 0, "Error reading file magic number");
  if (strcasecmp(ppm_type, PPM_TYPE))
    error(1, 0, "Error reading file: expected magic %s was %s", PPM_TYPE, ppm_type);
  ppm_read_whitespaces_or_comment(f);
  if (fscanf(f, "%d", &new.width) != 1)
    error(1, 0, "Error reading file: image width can't be read");
  ppm_read_whitespaces_or_comment(f);
  if (fscanf(f, "%d", &new.height) != 1)
    error(1, 0, "Error reading file: image height can't be read");
  ppm_read_whitespaces_or_comment(f);
  if (fscanf(f, "%hd", &new.depth) != 1)
    error(1, 0, "Error reading file: image depth can't be read");
  ppm_read_whitespaces_or_comment(f);
...
void ppm_save(t_ppm p, FILE *f)
{
  ...
  if (fprintf(f, "%s\n%d %d\n%d\n",
              ...
              p.height,
              p.depth) <= 0)
    error(1, 0, "Error writing PPM header");
